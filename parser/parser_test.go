...
func TestLetStatements(t *testing.T) {
	...
	// this lexer is package lexer ("monkey/lexer")
	l := lexer.New(input)
...
func TestIdentifierExpression(t *testing.T) {
	...
	// when Expression property is Identifier
	// Expression is duck type and contain multiple type like Identifier
	...
	if ident.TokenLiteral() != "foobar" {
		t.Errorf("ident.TokentLiteral not %s. got=%s", "foobar", ident.TokenLiteral())
	}
}
...
func TestParsingPrefixExpressions(t *testing.T) {
	...
	for _, tt := range prefixTests {
		...
		if len(program.Statements) != 1 {
			t.Fatalf("programs.Statements does not contain %d statements. got=%d\n", 1, len(program.Statements))
		}
		stmt, ok := program.Statements[0].(*ast.ExpressionStatement)
		if !ok {
			t.Fatalf("program.Statements[0] is not ast.ExpressionStatement. got=%T", program.Statements[0])
